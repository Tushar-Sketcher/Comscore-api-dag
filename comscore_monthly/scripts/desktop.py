...
# This report is to extract Desktop Only data source from https://api.comscore.com/KeyMeasures.asmx
# including one & three months average report

config_yaml = yaml.safe_load(open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'desktop.yaml')))


def get_report_id(month, time_type, base64string, media_set, media_set_type):
    ...
    </soapenv:Envelope>""".format(time_type=time_type, month=month, mediaset=media_set, mediasettype=media_set_type)
...
def get_data(month, time_type, base64string, media_set, media_set_type):
    raw_data = []
    processed_data = []
    job_id = get_report_id(month, time_type, base64string, media_set, media_set_type)
    ...
    for row in raw_data:
        if len(row) == len(header):
            # desktop report header added 'attribute' since 2019-07-01
            for i in range(6, len(raw_data[0])):
                if i >= len(row):
                    processed_data += [row[:6] + [header[i].text.strip(), header[i]['id'], 'None']]
                else:
                    processed_data += [row[:6] + [header[i].text.strip(), header[i]['id'], row[i]]]
        else:
            logger.info(row)
...
if __name__ == '__main__':
    ...
    for time_frame in time_frame_list:
        for category in categories:
            df = get_data(month_id, time_frame['id'], base64string, category['media_set'], category['media_set_type'])
            df['timeframe'] = time_frame['time']
            df['month'] = datetime.strptime(month, '%Y-%m-%d').strftime('%B')
            df['year'] = month[:4]
            df['year'] = pd.to_numeric(df["year"])
            df['report_type'] = 'Desktop Only'
