...
# This report is to extract Desktop Only data source from https://api.comscore.com/DigitalCalc.asmx?wsdl
# including one month average report

config_yaml = yaml.safe_load(open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'audience_duplication.yaml')))


def get_processed_df(processed_data, media):
    
    # create a dataframe
    df = pd.DataFrame(processed_data)
    df.columns = ['benchmark_media', 'benchmark_media_id', 'metrics', 'metric_id', 'benchmark_metric_value', 'demographic']
    df['benchmark_metric_value'] = pd.to_numeric(df['benchmark_metric_value'], errors='coerce')
    
    id = [str(m['id']) for m in media] 
    # segregate df with provided media
    with_media_df = df[df['benchmark_media_id'].isin(id)]
    
    # segregate df without provided media and pivot as per bussiness logic
    without_media_df = df[~df['benchmark_media_id'].isin(id)]
    without_media_df = without_media_df.pivot(index = ['benchmark_media_id','metrics','demographic','metric_id' ], columns=['benchmark_media'], values=['benchmark_metric_value']).reset_index()
    without_media_df.columns = ['benchmark_media_id', 'metrics', 'demographic','metric_id', 'duplicated_2_or_more_sites', 'duplicated_all_sites','unduplicated']

    # join dfs to get complete reshaped dataframe
    df = pd.merge(with_media_df, without_media_df[['metric_id','duplicated_all_sites','unduplicated']], how='left', on='metric_id')
    df['comparision_media'] = df['benchmark_media'].iloc[::-1].reset_index(drop=True)

    return df


def generate_parameters(month, time_type, target_type, measures, report):
    """
    ...
    for media in medias:
        xml += ['<rep:Parameter KeyId="media" Value="{id}"/>'.format(id=media['id'])]
...
def get_report_id(month, time_type, base64string, target_type, measures, report):
    """
    ...
    """
    xml = generate_parameters(month, time_type, target_type, measures, report)
    ...
    headers = {"POST": "/DigitalCalc.asmx",
               "Content-Type": "text/xml; charset=UTF-8",
               "Content-Length": str(len(encoded_request)),
               "SOAPAction": "http://comscore.com/SubmitReport",
               "Authorization": "Basic %s" % base64string}

    url = "https://api.comscore.com/DigitalCalc.asmx?wsdl"
...
def get_data(month, time_type, base64string, target_type, measures, report):
    """
    Function will fetch the crossvisiting data report through the job id with xml script
    :param month: month_id is time period.
    :param time_type: average time type eg: 1 Month Average.
    ...
    job_id = get_report_id(month, time_type, base64string, target_type, measures, report)
    # Use the job ID to fetch report https://api.comscore.com/DigitalCalc.asmx?wsdl?op=FetchReport
    ...
    header = [str(rows[0].find('th').text.strip().lower()), 'web_id'] + header

    # Loop through the metrics data to put one metric and one media as one row
    for row in raw_data:
        if len(row) == len(header):
            for i in range(2, len(raw_data[0])):
                processed_data += [row[:2] + [header[i].text.strip(), header[i]['id'], row[i], target.text.strip()]]
        else:
            logger.info(row)
    
    df = get_processed_df(processed_data, media)
...
if __name__ == '__main__':
    ...
    for time_frame in time_frame_list:
        for target_type in targets:
            for report in reports:
                df = get_data(month_id, time_frame['id'], base64string, target_type, measures, report)
    ...
    schema = StructType([StructField('benchmark_media', StringType(), nullable=True),
                         ...
                         StructField('metrics', StringType(), nullable=True),
                         StructField('metric_id', StringType(), nullable=True),
                         StructField('benchmark_metric_value', FloatType(), nullable=True),
                         StructField('demographic', StringType(), nullable=True),
                         StructField('duplicated_all_sites', FloatType(), nullable=True),
                         StructField('unduplicated', FloatType(), nullable=True),
                         StructField('comparision_media', StringType(), nullable=True),
